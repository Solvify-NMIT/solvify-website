...
import ByteBattleCard from "./ByteBattleCard";
...
const Event = () => {
  ...
  const eventData = useMemo(() => {
    const flattened: { id: string; name: string; description: string; img: string; }[] = [];
  ...
  const [xOffset, setXOffset] = useState(340);

  useEffect(() => {
    const handleResize = () => {
      setXOffset(window.innerWidth < 768 ? 290 : 340);
    };
  ...
  return (
    <div className="relative flex min-h-screen w-full flex-col items-center justify-center overflow-hidden bg-[#050505] text-white py-10">
      ...
      {/* 3D Carousel Container - Height adjusted to 380px to accommodate taller cards and scale effect */}
      <div className="relative w-full max-w-[1200px] h-[380px] flex items-center justify-center perspective-[1200px] z-10">
          ...
          {eventData.map((event, index) => {
            ...
            return (
              <motion.div
                ...
                className={`absolute w-[280px] md:w-[380px] h-[240px] md:h-[320px] cursor-pointer rounded-xl overflow-hidden border-[1px] ${isCenter ? "border-[#FFD700] shadow-[0_0_40px_rgba(255,215,0,0.2)]" : "border-white/10"
